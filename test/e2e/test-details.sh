...
RESP=$(curl -sf -X POST "$BASE/v1/details" \
    ...
    -d '{"title":"The Go Programming Language","author":"Alan A. A. Donovan","isbn":"978-0134190440","pages":380,"language":"English","publisher":"Addison-Wesley"}')
HTTP_CODE=$(curl -s -o /dev/null -w "%{http_code}" -X POST "$BASE/v1/details" \
    -H "Content-Type: application/json" \
    -d '{"title":"The Go Programming Language 2","author":"Alan A. A. Donovan","isbn":"978-0134190441","pages":380,"language":"English","publisher":"Addison-Wesley"}')
...
[ "$COUNT" -ge 1 ] || { echo "FAIL: expected at least 1 book in list, got $COUNT"; exit 1; }

# Missing required field (empty title) should return 400
HTTP_CODE=$(curl -s -o /dev/null -w "%{http_code}" -X POST "$BASE/v1/details" \
    -H "Content-Type: application/json" \
    -d '{"title":"","author":"Test Author"}')
